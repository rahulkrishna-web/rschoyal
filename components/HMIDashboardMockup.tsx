...
import { Play, Square, Settings, ShieldAlert, CheckCircle2, Flame, Zap, ArrowUpRight } from "lucide-react";

export default function HMIDashboardMockup() {
  const [isRunning, setIsRunning] = useState(true);
  const [rpm, setRpm] = useState(280);
  const [temp, setTemp] = useState(42);
  const [power, setPower] = useState(11.2);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setPower((p) => Math.min(15, Math.max(9, p + (Math.random() - 0.5) * 0.4)));
    }, 2000);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:h-[calc(100%-80px)] h-auto">
        ...
        <div className="space-y-4 flex flex-col">
          ...
          <div className="bg-slate-950/40 border border-slate-800/80 rounded-2xl p-4 h-24 flex items-center justify-between">
            <div>
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Stone Temp</span>
              <span className="text-2xl font-black text-slate-100 mt-1 block">
                {isRunning ? temp.toFixed(1) : "0.0"}°C
              </span>
            </div>
            ...
            <div>
              ...
              <div className="flex items-baseline space-x-1.5 mt-2">
                <span className="text-3xl font-black tracking-tight text-gradient-accent">
                  {isRunning ? power.toFixed(1) : "0.0"}
                </span>
                <span className="text-xs font-mono text-slate-400 uppercase">kW/h</span>
              </div>
              
              {/* Progress bar visual */}
              <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden mt-4">
                <div 
                  className="bg-gradient-to-r from-brand-indigo to-brand-cyan h-full transition-all duration-500" 
                  style={{ width: `${isRunning ? (power / 15) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
